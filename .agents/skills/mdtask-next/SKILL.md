...
2. Confirm the task has no unresolved portfolio blockers and review its exact execution file with `./taskctl show <task-id>`. A task missing from `ready` only because of a qualified cross-repository blocker (`owner/repo#ID`) may still be startable: check `./taskctl federation ready --peer-root <RIPDPI checkout>` to see whether the peer resolves it.
