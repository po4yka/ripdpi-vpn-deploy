...
OWNERSHIP_OWNED = ('sshd_config', *OWNED)
BASELINE_OWNED = ('sshd_config', OWNED[1])
BASELINE_FILE_GID = 0
MAX_HARDENING = 8192
MAX_FILE = 256 * 1024
...
def _historical_plan(plan):
    """Validate only the exact ownership-only plan emitted by schema v1."""
    try:
        ...
        for name, pair in plan['files'].items():
            ...
            for value in pair.values():
                _record(value)
            if any(pair['before'][key] != pair['after'][key]
                   for key in ('exists', 'mode', 'uid', 'gid')):
                raise ValueError
        ...
        for item in reads:
            ...
            if any(type(item[key]) is not int or item[key] < 0
                   for key in ('size', 'mode', 'uid', 'gid', 'dev', 'ino')):
                raise ValueError
            if (item['size'] > MAX_FILE or item['mode'] > 0o777
                    or item['mode'] & 0o022 or item['uid'] != os.geteuid()):
                raise ValueError
            if name in plan['files']:
                before = plan['files'][name]['before']
                if (not before['exists'] or any(item[key] != before[key]
                                                for key in ('sha256', 'mode', 'uid', 'gid'))):
                    raise ValueError
            names.append(name)
        if len(set(names)) != len(names) or 'sshd_config' not in names:
            raise ValueError
        if any(pair['before']['exists'] != (name in names)
               for name, pair in plan['files'].items()):
            raise ValueError
        inventory = [{'relative_directory': 'sshd_config.d',
                      'matched_names': sorted(Path(name).name for name in names
                                              if name != 'sshd_config')}]
...
def _plan(plan):
    try:
        ...
        owned = {'sshd-ownership': OWNERSHIP_OWNED, 'sshd-baseline': BASELINE_OWNED}[operation]
        if (set(plan) != {'schema_version', 'operation', 'changed', 'read_set', 'include_inventory', 'files', 'effective', 'snapshot_digest'}
                or type(plan['schema_version']) is not int or plan['schema_version'] != 2
                or type(plan['changed']) is not bool or set(plan['files']) != set(owned)):
            raise ValueError
        _hex(plan['snapshot_digest'])
        if _digest({k: v for k, v in plan.items() if k != 'snapshot_digest'}) != plan['snapshot_digest']:
            raise ValueError
        for name, pair in plan['files'].items():
            ...
            if operation == 'sshd-ownership':
                # Ownership-only migration never creates/deletes files or
                # changes metadata. Missing cloud-init 50 remains missing.
                if any(before[k] != after[k] for k in ('exists', 'mode', 'uid', 'gid')):
                    raise ValueError
                if name != OWNED[2] and not before['exists']:
                    raise ValueError
            else:
                if not after['exists'] or (name == 'sshd_config' and not before['exists']):
                    raise ValueError
                if before['exists']:
                    if any(before[k] != after[k] for k in ('mode', 'uid', 'gid')):
                        raise ValueError
                elif (name != OWNED[1] or after['mode'] != 0o644
                      or after['uid'] != os.geteuid() or after['gid'] != BASELINE_FILE_GID):
                    raise ValueError
                if name == OWNED[1] and not 0 < len(_record(after)) <= MAX_HARDENING:
                    raise ValueError
        reads = plan['read_set']
        minimum_reads = 2 if operation == 'sshd-baseline' else 3
        if not isinstance(reads, list) or not minimum_reads <= len(reads) <= 65:
            raise ValueError
        names = []
        for item in reads:
            if set(item) != {'relative_path', 'sha256', 'size', 'mode', 'uid', 'gid', 'dev', 'ino'}:
        ...
        if len(set(names)) != len(names) or 'sshd_config' not in names:
            raise ValueError
        if operation == 'sshd-baseline' and OWNED[0] not in names:
            raise ValueError
        ...
        for index, item in enumerate(plan['effective']):
            ...
            if index:
                _context(item['context'])
            _hex(item['before_sha256'])
            _hex(item['after_sha256'])
            if operation == 'sshd-ownership' and item['before_sha256'] != item['after_sha256']:
                raise ValueError
        if (plan['effective'][0]['context'] is not None
                or len({_digest(item['context']) for item in plan['effective']}) != len(plan['effective'])):
            raise ValueError
...
class Transaction:
    ...
    def _load(self):
        ...
        if not os.path.lexists(path):
            ...
            return None
        try:
            encoded = _read(path, private=True, limit=MAX_STATE)[0]
            ...
            if set(value) != {'schema_version', 'generation', 'nonce', 'created', 'deadline', 'monotonic_created', 'monotonic_deadline', 'boot_id', 'status', 'plan', 'checksum'}:
                raise ValueError
            if type(value['schema_version']) is not int or value['status'] not in STATES:
                raise ValueError
            ...
            if value['schema_version'] == 1:
                if value['status'] not in TERMINAL:
                    raise TransactionError('historical-state-pending')
                _historical_plan(value['plan'])
                if encoded != _json(value):
                    raise ValueError
    ...
    def _archive(self, state):
        source = _read(self.root / 'transaction.json', private=True, limit=MAX_STATE)[0]
        if source != _json(state):
            raise TransactionError('state-invalid')
        destination = self.root / (state['generation'] + '.json')
        if os.path.lexists(destination):
            if _read(destination, private=True, limit=MAX_STATE)[0] != source:
                raise TransactionError('state-invalid')
            return
        _atomic(destination, source)

    @staticmethod
    def _mutable(state):
        if state is not None and state['schema_version'] == 1:
            raise TransactionError('historical-state-terminal')
    ...
    def _graph(self, plan, phase):
        ...
        if phase not in {'before', 'after', 'mixed'}:
            raise TransactionError('phase-invalid')
        directory = self.config / 'sshd_config.d'
        _directory(directory)
        names = sorted(p.name for p in directory.glob('*.conf') if not p.name.startswith('.'))
        before_names = set(plan['include_inventory'][0]['matched_names'])
        after_names = set(before_names)
        for name, pair in plan['files'].items():
            if name == 'sshd_config':
                continue
            if pair['after']['exists']:
                after_names.add(Path(name).name)
            else:
                after_names.discard(Path(name).name)
        permitted = [before_names, after_names] if phase == 'mixed' else [before_names if phase == 'before' else after_names]
        if set(names) not in permitted:
    ...
    def _publish(self, name, value):
        if name not in {*OWNERSHIP_OWNED, *BASELINE_OWNED}:
    ...
    def prepare(self, *, intent, contexts, hardening=None, timeout=180):
        ...
        with self._locked(create=True):
            ...
            state = dict(schema_version=2, generation=str(uuid4()), nonce=secrets.token_hex(32),
    ...
    def apply(self, generation, nonce):
        ...
        with self._locked():
            ...
            self._save(state)
            try:
                owned = {'sshd-ownership': OWNERSHIP_OWNED,
                         'sshd-baseline': BASELINE_OWNED}[state['plan']['operation']]
                for name in owned:
                    pair = state['plan']['files'][name]
                    self._live(state)
                    self._graph(state['plan'], 'mixed')
                    if pair['before'] != pair['after']:
                        self._publish(name, pair['after'])
                self._graph(state['plan'], 'after')
                self.runtime.assert_effective(state['plan'], self.config, phase='after')
                self._live(state)
                self.runtime.reload()
                self._live(state)
    ...
    def _rollback(self, state, *, boot):
        try:
            ...
            self._graph(state['plan'], 'before')
            self.runtime.assert_effective(state['plan'], self.config, phase='before')
            if not boot:
                self.runtime.reload()
