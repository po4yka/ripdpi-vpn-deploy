...
> Redaction note (2026-08): the operator-owned production target measured in this document is a
> registered domain that is deliberately not named here. Every occurrence of `<redacted-domain>`
> refers to that one undisclosed real target; the DNS records, certificate properties, probe
> results, and dates quoted below were observed against it exactly as stated.
## Decision

The best production target for the P0 node is an operator-owned self-steal endpoint named `edge.<redacted-domain>`: publish an A record to the P0 address, keep Xray on public TCP/443, terminate a real TLS 1.3 + HTTP/2 site on `127.0.0.1:8443`, set `xray.target` to that local listener, and use only `edge.<redacted-domain>` in `xray.server_names`. Do not publish AAAA until the IPv6 listener and provider/host firewall path have been tested.
...
|---|---:|---|---|
| `edge.<redacted-domain>` self-steal | Not yet deployable/testable at research time | Same P0 IP and AS202053 by design | Selected; implementation status is recorded above |
| Then-current `<redacted-domain>` target | 0 failures, 0 warnings | HTTP 200; TLS 41–42 ms; AS12876 target | Superseded by self-steal |
...
No OONI measurements exist yet for `<redacted-domain>`, which is expected for a new low-profile domain. Absence of measurements is not evidence of either reachability or blocking.
...
## Promotion gates for `edge.<redacted-domain>`
...
1. Replace the current wildcard-derived `edge.<redacted-domain> CNAME <redacted-domain>` behavior with an explicit A record to the P0 UpCloud IPv4 address. Leave AAAA absent until equivalent IPv6 behavior is proven.
2. Issue a public certificate whose SAN includes exactly `edge.<redacted-domain>`; store it under the existing local SOPS boundary.
...
4. Set `xray.target` to the private listener and `xray.server_names` to `edge.<redacted-domain>`; keep public Xray on TCP/443.
