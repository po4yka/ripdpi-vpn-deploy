...
Source: repository-local subscription requirements.

This document compares the documented subscription-delivery
...
| Property | Target | This repo today | Gap |
|---|---|---|---|
| Per-device opaque token | required | required for `/sub/` and `vpnd share`; generated by `issue-sub-token.sh` | ✓ shipped |
| Token stored as hash, not plaintext | required | disk paths use `<route>/<sha256(token)>`; plaintext token never touches the server filesystem | ✓ shipped |
...
the gap matrix above is at parity with the documented contract. Remaining work
...
- No HTML rendering on subscription endpoints.
- No JavaScript on subscription endpoints.
- No browser-style cookies.
- Plain static payload, no API.
