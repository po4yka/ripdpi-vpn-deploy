...
| **Ansible role: amneziawg** | ansible-lint | render check | **molecule** (real role + idempotence + verify) | synthetic local Git/Make inputs and no-TUN tools exercise role-owned builds, receipts, config and systemd lifecycle. The isolated x86_64 QEMU-backed scenario passed converge, a zero-change second converge, verify and destroy. No upstream build or tunnel proof. |
