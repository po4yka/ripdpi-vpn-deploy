...
| **Python unit tests (622 collected)** | pytest (CI) | n/a | n/a | Covers emit-singbox, SOPS round-trip, render-inventory including Vultr guest IPv4 convergence, relay/fallback, subscription token revocation lifecycle, probe-matrix drivers/contracts/provisioning, cascade attestation freshness, tri-state per-connection classification, authenticated per-leg probe state and health freshness, inert Terraform and role/profile guards, tspu-canary, Snell refinement classification, singbox kill-switch, policy-ratelimit ban logic, listener-collision guard, node manifest, AmneziaWG arm64 version-floor tracking, cloud-init completion, deployment profiles, watchdog bounded recovery, honeypot log retention, calendar-window metrics, dual-stack binding, public-site behavior, XHTTP-only backend rendering, REALITY self-steal contracts, repository symlink scanning, and protocol-liveness evaluation, sentinel cleanup, onboarding, and OTP rotation gates. (Shell orchestrator dry-runs migrated to bats — see below.) |
