...
| **Ansible role: amneziawg** | ansible-lint | render check | **molecule** (template converge + idempotence + verify) | real config and instance-target templates; no daemon activation or TUN proof. Runtime lifecycle requires a real VPS. |
...
| `make ci-fast` (operator) | Portable credential-free CI jobs: actionlint, strict offline zizmor, cloud-init schema, all provider Terraform tests, yamllint, shellcheck, cargo-deny, MSRV, render/schema/unit/bats, clippy, and Rust tests. Missing or wrong-version tools fail closed. |
