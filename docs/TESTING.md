...
| **Jinja2 templates (39)** | **`scripts/check-templates-render.py`** | renders all 39 role templates against synthetic + example data | n/a | Catches Jinja syntax errors, JSON-invalid output for `*.json.j2`, nginx parse errors for site configs. |
...
| **Python unit tests (157 tests, 1 skip)** | pytest (CI) | n/a | n/a | Covers emit-singbox, SOPS round-trip, render-inventory, relay/fallback, subscription token revocation lifecycle, tspu-canary, scan-reality-targets, singbox kill-switch, policy-ratelimit ban logic, listener-collision guard, node manifest, and deployment profiles. (Shell orchestrator dry-runs migrated to bats — see below.) |
...
| **Jinja2 snapshot diff (39 templates)** | `scripts/render-snapshots.py` | golden-file diff | n/a | Fails on any unintended render change. Run `make snapshot-update` after intentional template edits. |
