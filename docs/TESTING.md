...
Renovate is the sole automated version-update PR owner. `.github/dependabot.yml` is intentionally absent to prevent duplicate update PRs, and the `enabledManagers` allowlist in `renovate.json` is the canonical ownership boundary.
