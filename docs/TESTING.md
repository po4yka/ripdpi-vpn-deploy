...
| **Ansible role: observability_deadman** | focused pulse, replay, state, delivery and privacy tests | JSON render + systemd templates | **molecule** (default + required `enabled`) | Default-off refusal and enabled generation/idempotence/removal scenarios; secondary Telegram delivery and staged rollback still require live acceptance. |
...
| **Jinja2 templates (128)** | **`scripts/check-templates-render.py`** | renders all 128 role templates against synthetic + example data | n/a | Catches Jinja syntax errors, JSON-invalid output for `*.json.j2`, nginx parse errors for site configs. |
...
| **Jinja2 snapshot diff (137 templates)** | `scripts/render-snapshots.py` | golden-file diff | n/a | Fails on any unintended render change. Run `make snapshot-update` after intentional template edits. |
...
### Protective workflow merge gate

Main CI calls `tf-policy`, `image-scan`, `contract-sync` and `reproducible-build`
on every run and includes their results in the branch-required `required checks`
job. Only success is accepted; failure, cancellation and skips block merge.
The existing standalone manual triggers remain, but PR execution has one owner
in `ci.yml`. This makes existing outcomes mandatory; it does not turn advisory
Xray rebuild comparisons or placeholder example pins into reproducibility proof.
...
the portable `ci-fast` / `check` bundle: never elevate the full local test suite.
