...
| **Python unit tests (478 collected)** | pytest (CI) | n/a | n/a | Covers emit-singbox, SOPS round-trip, render-inventory including Vultr guest IPv4 convergence, relay/fallback, subscription token revocation lifecycle, probe-matrix drivers/contracts/provisioning, tspu-canary, Snell refinement classification, scan-reality-targets, singbox kill-switch, policy-ratelimit ban logic, listener-collision guard, node manifest, AmneziaWG arm64 version-floor tracking, cloud-init completion, deployment profiles, honeypot log retention, calendar-window metrics, dual-stack binding, and protocol-liveness evaluation, sentinel cleanup, onboarding, and OTP rotation gates. (Shell orchestrator dry-runs migrated to bats — see below.) |
