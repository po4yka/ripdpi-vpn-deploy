...
| **Jinja2 templates (53)** | **`scripts/check-templates-render.py`** | renders all 53 role templates against synthetic + example data | n/a | Catches Jinja syntax errors, JSON-invalid output for `*.json.j2`, nginx parse errors for site configs. |
...
| **Python unit tests (485 collected)** | pytest (CI) | n/a | n/a | Covers emit-singbox, SOPS round-trip, render-inventory, relay/fallback, subscription token revocation lifecycle, probe-matrix and Hysteria Gecko evidence drivers/contracts/provisioning, tspu-canary, Snell refinement classification, scan-reality-targets, singbox kill-switch, policy-ratelimit ban logic, listener-collision guard, node manifest, AmneziaWG arm64 version-floor tracking, deployment profiles, and protocol-liveness evaluation, sentinel cleanup, onboarding, and OTP rotation gates. (Shell orchestrator dry-runs migrated to bats — see below.) |
...
| **Jinja2 snapshot diff (56 templates)** | `scripts/render-snapshots.py` | golden-file diff | n/a | Fails on any unintended render change. Run `make snapshot-update` after intentional template edits. |
