...
| **Jinja2 snapshot diff (139 templates)** | `scripts/render-snapshots.py` | golden-file diff | n/a | Fails on any unintended render change. Run `make snapshot-update` after intentional template edits. |
...
`tests/unit/` and `scripts/tests/` (including all 54 taskctl regressions);
