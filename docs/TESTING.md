...
| **Ansible role: network-exposure-gate** | signed-artifact schema and trust checks | redacted review; disabled/log-only firewall parity | **molecule** (converge + idempotence + verify + rollback) | Detached RSA-SHA256 signature against an operator-pinned public key, explicit host/digest promotion, separate ingress/host-egress/forwarded policy. No feed updater; real staging remains a separate acceptance gate. |
| **Ansible role: firewall** | ansible-lint | render check | **molecule** | tests nftables.conf parse + presence of expected ports. |
| **Ansible role: xray** | ansible-lint | render check (JSON validity) | **molecule** (converge + idempotence + verify; fixture binary) | exercises the runtime role, config.json shape, systemd unit, and symlink rollback; upstream binary/data-plane acceptance is separate. |
...
| **Ansible role: amneziawg** | ansible-lint | render check | **molecule** (real role converge + idempotence + verify; fixture runtime) | executes source/build-receipt guards, config, unit, and lifecycle tasks with fixture binaries; no real tunnel or TUN proof. Runtime lifecycle requires a real VPS. |
...
| **Ansible role: geodata** | ansible-lint | render check | **molecule** (converge + verify) | real downloader and sandboxed refresh service use pinned local fixture sources; upstream availability and an active Xray daemon remain live checks. |
...
| **Ansible role: warp-outbound** | ansible-lint | render check | **molecule** (converge + verify) | command fixtures exercise the actual health and signing-key gates; real registration, tunnel egress, and filtered-path reachability remain live checks. |
...
| **Full stack** | ansible-lint | render check | **required CI matrix** (`full-stack` and `full-stack-published`) | runs `site.yml`, a second idempotent converge, and enabled service/listener checks in privileged Debian-13 containers. The published scenario additionally verifies the same role contract with host port mappings. See `ansible/molecule/full-stack/`. |
...
| **Jinja2 templates (97)** | **`scripts/check-templates-render.py`** | renders all 97 role templates against synthetic + example data | n/a | Catches Jinja syntax errors, JSON-invalid output for `*.json.j2`, nginx parse errors for site configs. |
...
| **Python unit tests (1164 collected)** | pytest (CI and local) | n/a | n/a | Covers emit-singbox, SOPS round-trip, render-inventory including custom SSH ports, replacement safety, Vultr control-plane preflight and guest IPv4 convergence, rolling OS maintenance and recurring security-update policy, relay/fallback, subscription token revocation lifecycle, client registry refresh/drift contracts, probe-matrix drivers/contracts/provisioning, cascade attestation freshness, tri-state per-connection classification, authenticated per-leg probe state and health freshness, inert Terraform and role/profile guards, in-cohort rule-drift canary classification, Snell refinement classification, singbox kill-switch, policy-ratelimit ban logic, burn-check error metrics, listener-collision guard, node manifest source parity, AmneziaWG check-mode build receipts and arm64 version-floor tracking, recurring real-VPS AWG/NAT source binding, generation transitions, transactional rotation, cleanup, and fail-closed evidence, cloud-init completion, deployment profiles, strict live-inventory and extra-vars gates, injected-fact removal, watchdog bounded recovery, Xray StatsService and redacted exporter contracts, check-mode-safe service handlers, Molecule dependency-path and driver-pin guards, honeypot log retention, calendar-window metrics, dual-stack binding, public-site behavior, P1 node-local hostname resolution, XHTTP-only backend rendering, REALITY self-steal contracts, repository symlink scanning, and protocol-liveness evaluation, sentinel cleanup, onboarding, and OTP rotation gates. (Shell orchestrator dry-runs migrated to bats — see below.) |
| **Shell-orchestrator bats tests (50 tests)** | `bats tests/bats/` (CI, blocking) | n/a | n/a | Covers `blue-green.sh --dry-run`, `fleet-rotate.sh --dry-run`, `age-recovery-combine.sh` 3-of-5 round-trip, `restore.sh --dry-run` (path-A + path-B including secrets-before-playbook ordering), operator cron rendering, and atomic optional Snell client updates. Uses the same `tests/stubs/bin/` PATH-prepend harness as the Python tests. bats-support v0.3.0 + bats-assert v2.1.0 vendored under `tests/bats/test_helper/`. |
| **Terraform policy (cross-provider, Conftest)** | `.github/workflows/tf-policy.yml` per PR | n/a | n/a | Runs each provider's native Terraform tests with `mock_provider`, then `conftest verify --rego-version v0 -p terraform/policy/` for Rego syntax/unit-test validation. The workflow intentionally does not run real provider plans against example tfvars because those require operator credentials and provider API access. `make tf-policy-verify` runs the same policy tests locally and is included in `make ci-fast` / `make check`; Conftest 0.57.0 is pinned in mise. `make tf-policy` additionally exercises all provider mocks. |
...
| **Jinja2 snapshot diff (102 templates)** | `scripts/render-snapshots.py` | golden-file diff | n/a | Fails on any unintended render change. Run `make snapshot-update` after intentional template edits. |
...
| `git push` (PR) | CI matrix: terraform fmt+validate (4 providers plus the inert exception root), terraform test (4 providers), cloud-init schema, ansible-lint + syntax, default Molecule scenarios for `baseline`, `firewall`, `network-exposure-gate`, `xray`, `hysteria`, `nginx-xhttp`, `watchdog`, `monitoring`, `backup`, `subscription-host`, `amneziawg`, `geodata`, `cascade-egress`, `cdn-front`, `warp-outbound`, `hysteria-realm`, `honeypot`, `dns-morph-bridge`, and `split-hop-egress`; non-default scenarios `watchdog/failure`, `cascade-ingress/populated`, `cascade-ingress/forced-empty`, and `hysteria-realm/shared-tls`; shellcheck, secrets-coverage, templates-render, yamllint, gitleaks, strict offline zizmor, `pytest tests/unit/` (live collection count recorded above), Rust tests, bats tests, Conftest TF policy, Trivy image scan, snapshot diff, and secrets schema. |
...
| `make dry-run` / `make deploy` / `make verify` | **pre-deploy-check** validates secrets and certificates. Deploy and dry-run then require bounded cloud-init success plus the bootstrap marker for every `HOSTS` pair; these readiness checks have no skip flag. |
