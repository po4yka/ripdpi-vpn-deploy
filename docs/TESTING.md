...
| **Ansible role: amneziawg** | ansible-lint | render check | **molecule** (default sequence: dependency, syntax, create, prepare, converge, idempotence, verify, destroy) | synthetic local Git inputs compile a C awg fixture; no-TUN tools exercise role-owned builds, receipts, config, systemd lifecycle and idempotence. No upstream build or tunnel proof. |
...
| **Ansible role: naive** | ansible-lint | render check (Caddyfile syntax NOT validated — Caddy not in CI matrix) | **molecule** (default sequence: dependency, syntax, create, prepare, converge, side_effect, verify, destroy) | xcaddy from-source build skipped in CI to avoid Go-module-network flakiness; `molecule converge` runs it locally. |
...
| **Jinja2 templates (140)** | **`scripts/check-templates-render.py`** | renders all 140 role templates against synthetic + example data | n/a | Catches Jinja syntax errors, JSON-invalid output for `*.json.j2`, nginx parse errors for site configs. |
...
`tests/unit/` and `scripts/tests/` (including all 55 taskctl regressions);
