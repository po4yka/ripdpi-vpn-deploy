...
| **Ansible role: cascade-ingress** | ansible-lint + structural guard tests | render check + classifier unit fixtures | **required paired molecule** (`populated` + `forced-empty`) | EXCEPTION-tier, absent from family profiles, service management default-off. The forced-empty case must block before tunnel configuration or serving state and is required independently from the happy path. |
...
| **Jinja2 templates (64)** | **`scripts/check-templates-render.py`** | renders all 64 role templates against synthetic + example data | n/a | Catches Jinja syntax errors, JSON-invalid output for `*.json.j2`, nginx parse errors for site configs. |
...
| **Jinja2 snapshot diff (64 templates)** | `scripts/render-snapshots.py` | golden-file diff | n/a | Fails on any unintended render change. Run `make snapshot-update` after intentional template edits. |
