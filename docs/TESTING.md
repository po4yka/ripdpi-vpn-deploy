...
| **Python unit tests (453 collected)** | pytest (CI) | n/a | n/a | Covers emit-singbox, SOPS round-trip, render-inventory, relay/fallback, subscription token revocation lifecycle, probe-matrix drivers/contracts/provisioning, tspu-canary, scan-reality-targets, singbox kill-switch, policy-ratelimit ban logic, listener-collision guard, node manifest, AmneziaWG arm64 version-floor tracking, deployment profiles, and protocol-liveness evaluation, sentinel cleanup, onboarding, and OTP rotation gates. (Shell orchestrator dry-runs migrated to bats — see below.) |
...
| **Jinja2 snapshot diff (53 templates)** | `scripts/render-snapshots.py` | golden-file diff | n/a | Fails on any unintended render change. Run `make snapshot-update` after intentional template edits. |
