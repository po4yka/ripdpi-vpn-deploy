...
| **Ansible role: node_manifest** | ansible-lint | render check + snapshot | n/a | writes `/var/lib/ripdpi-vpn-deploy/manifest.json`; unit tests assert public listener and security-control fields stay non-secret and parseable. |
...
| **Shell scripts (52)** | `bash -n` syntax + **`shellcheck -s bash -S warning`** (CI) | n/a | n/a | every shell script in `scripts/` runs through shellcheck; the two isolated exception-root boundaries are linted by the exception Terraform CI job. |
...
| **Jinja2 templates (67)** | **`scripts/check-templates-render.py`** | renders all 67 role templates against synthetic + example data | n/a | Catches Jinja syntax errors, JSON-invalid output for `*.json.j2`, nginx parse errors for site configs. |
...
| **Python unit tests (537 collected)** | pytest (CI) | n/a | n/a | Covers emit-singbox, SOPS round-trip, render-inventory including Vultr guest IPv4 convergence, relay/fallback, subscription token revocation lifecycle, probe-matrix drivers/contracts/provisioning, cascade attestation freshness, tri-state per-connection classification, authenticated per-leg probe state and health freshness, inert Terraform and role/profile guards, tspu-canary, Snell refinement classification, singbox kill-switch, policy-ratelimit ban logic, listener-collision guard, node manifest, AmneziaWG arm64 version-floor tracking, cloud-init completion, deployment profiles, watchdog bounded recovery, honeypot log retention, calendar-window metrics, dual-stack binding, and protocol-liveness evaluation, sentinel cleanup, onboarding, and OTP rotation gates. (Shell orchestrator dry-runs migrated to bats — see below.) |
| **Shell-orchestrator bats tests (46 tests)** | `bats tests/bats/` (CI, blocking) | n/a | n/a | Covers `blue-green.sh --dry-run`, `fleet-rotate.sh --dry-run`, `age-recovery-combine.sh` 3-of-5 round-trip, `restore.sh --dry-run` (path-A + path-B including secrets-before-playbook ordering), and atomic optional Snell client updates. Uses the same `tests/stubs/bin/` PATH-prepend harness as the Python tests. bats-support v0.3.0 + bats-assert v2.1.0 vendored under `tests/bats/test_helper/`. |
...
| **Jinja2 snapshot diff (67 templates)** | `scripts/render-snapshots.py` | golden-file diff | n/a | Fails on any unintended render change. Run `make snapshot-update` after intentional template edits. |
...
| `git push` (PR) | CI matrix: terraform fmt+validate (3 providers plus the inert exception root), terraform test (3 providers), cloud-init schema, ansible-lint + syntax, default Molecule scenarios for `baseline`, `firewall`, `xray`, `hysteria`, `nginx-xhttp`, `watchdog`, `monitoring`, `backup`, `subscription-host`, `amneziawg`, `geodata`, and `cascade-egress`; non-default scenarios `watchdog/failure`, `cascade-ingress/populated`, and `cascade-ingress/forced-empty`; shellcheck, secrets-coverage, templates-render, yamllint, gitleaks, `pytest tests/unit/` (live collection count recorded above), 123 Rust tests, 46 bats tests, Conftest TF policy, Trivy image scan, snapshot diff, and secrets schema. |
...
| `make bootstrap-dev` (first-time setup) | Serially installs portable mise pins, hashed Python requirements and hooks, the Rust 1.88.0 MSRV, then runs the CI-parity preflight. Cloud-init is a manual Ubuntu dependency; on macOS use an Ubuntu VM/environment or CI and never skip the schema gate. |
| `make check-prereqs CI_PARITY=1` (operator) | Aggregates every missing base and portable CI-parity executable, Python import, and MSRV toolchain in one report before failing. |
