...
| **Jinja2 templates (79)** | **`scripts/check-templates-render.py`** | renders all 79 role templates against synthetic + example data | n/a | Catches Jinja syntax errors, JSON-invalid output for `*.json.j2`, nginx parse errors for site configs. |
...
| **Jinja2 snapshot diff (79 templates)** | `scripts/render-snapshots.py` | golden-file diff | n/a | Fails on any unintended render change. Run `make snapshot-update` after intentional template edits. |
