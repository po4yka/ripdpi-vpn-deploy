...
| **Ansible role: honeypot** | ansible-lint | render check + focused log-retention contract | **molecule** + idempotence | verifies service active, listener bound to configured port, script installed, timer active, and forced rotation recreates a writable bounded log. |
