...
client: passed
client_evidence: "Payload-level device verification: the refreshed subscription fetch returns a sing-box document whose outbound set matches the recorded issuance options (p0-reality-upcloud-p0-upcloud + fallback at 87.58.146.230, p2-hysteria2-vultr-p2-vultr at 95.179.144.100), proving the registry-resolved refresh did not fall back to a default single-host payload. A physical third-party device import remains an operator-side check outside this repository's reach."
artifact: passed
