...
- Temporary staging cleanup uses separate provider-specific stdlib guards behind one literal-safe Make/destroy lifecycle rather than weakening generic destroy. Both adapters create a canonical mode-0600 manifest directly from exact private Terraform state bytes, bind the authenticated provider account, exact workspace/state digest and hostname, and derive fixed target/escalation/hard deadlines at 36/44/47 hours from the provider creation timestamp rather than operator input. The UpCloud adapter binds the server, computed root storage and server-bound firewall rules. The Vultr adapter independently binds the exact instance, SSH key, firewall group and every `for_each` ICMP, SSH and public-listener firewall rule. Vultr rule IDs remain provider-native positive decimals, and SSH rules bind the configured `terraform_data.ssh_port` value; its root is embedded as `{kind: instance-root, server_id, separate_storage_id: null}` rather than inventing a separate storage resource. The two adapters and their credential modes remain separate: the Vultr path accepts only ambient `VULTR_API_KEY`, the UpCloud path never inherits it, and all Make command-line credentials are rejected before expansion.
- Authorization and evidence reservation bind the same manifest/state inodes and bytes; plan validation requires that exact reservation, and an immediate pre-apply transition rechecks account, reservation, state and a freshly read exclusive hard deadline before persisting `apply_started`. Every private path is traversed component-by-component through held directory descriptors without following symlinks; release and rewrite recheck the exact opened inode. A durable transition journal recovers a complete, partial or not-yet-started in-place evidence update while preserving the originally reserved evidence inode; `reserved` and `plan_validated` states may be released before apply, while `apply_started` cannot. The private binary plan is opened once, unlinked, inspected through the provider/workspace router and applied through the same inherited descriptor. After apply the selected adapter reauthenticates the exact provider account, records typed categorical absence for every bound resource in the same evidence inode and only then emits a redacted audit event. Post-expiry read-only finalization is allowed only for an apply proven to have started before expiry and is labeled expired rather than on-time; cumulative billing history is not misreported as an immediate invoice reversal. Guarded staging preserves the shared generated inventory. Neither guard creates resources, changes provider policy, retries destructive actions or enables the blocked provider firewall.
