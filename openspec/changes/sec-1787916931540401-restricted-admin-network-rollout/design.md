...
- Temporary staging cleanup uses a separate stdlib guard rather than weakening generic destroy. It creates a canonical mode-0600 manifest directly from the exact private Terraform state bytes, binds the exact workspace/state digest, hostname, server UUID and computed root-storage UUID, and accepts only a delete-only plan for the server, its server-bound firewall rules and Terraform's local SSH-port identity. The command requires the manifest environment to match the exact destroy target and reserves the evidence inode before any Terraform action. The private binary plan is opened once, unlinked, inspected and applied through the same inherited descriptor so a pathname cannot substitute it after validation. After apply the guard performs bounded authenticated UpCloud GETs and records only categorical server/storage absence in the reserved inode; cumulative billing history is not misreported as an immediate invoice reversal. Manifest, state, plan view and evidence are no-follow same-owner private files. The guard never creates resources, changes provider policy, retries destructive actions or enables the blocked provider firewall.
