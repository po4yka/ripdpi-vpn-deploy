...
- **THEN** cleanup requires a private canonical manifest bound to the authenticated provider API username, exact Terraform workspace and state digest, hostname, server UUID, root-storage UUID, creation time and exact target/escalation/hard deadlines at 36, 44 and 47 hours.
...
- **AND** the authenticated API username MUST match the manifest before evidence reservation, provider refresh, plan or apply, and MUST be rechecked before post-destroy resource absence lookups.
