...
- **AND** the UpCloud adapter binds the exact server, computed root storage and server-bound firewall rules; the separate Vultr adapter binds the exact instance, SSH key, firewall group and every `for_each` ICMP, SSH and public-listener firewall rule, with the instance root represented only by its server ID and a null separate-storage ID.
