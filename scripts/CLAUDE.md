...
**Real-VPS AWG evidence is generation-bound and transactional** — the scheduled lane deploys the exact workflow archive, requires healthy direct TCP+UDP controls before an AWG failure is classified as product-facing, observes service/config generation changes, rejects the old PSK, and commits or rolls back the client/server pair. Its private hooks are fixed-command, root-owned boundaries; exit 75 means infrastructure unavailable, and only strict counters, enum verdicts, hashed identities, and digests may leave the sentinel.
