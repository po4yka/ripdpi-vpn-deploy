"""Shared fail-closed parser and route decision logic for V2Ray geoip.dat."""
...
def classify_address(networks: list[IPNetwork], destination: str) -> str:
    try:
        address = ipaddress.ip_address(destination)
    except ValueError as exc:
        raise DatasetUnavailable("destination is unknown or invalid") from exc
    return "ru" if any(address.version == network.version and address in network for network in networks) else "foreign"


def classify(path: Path, destination: str) -> dict[str, str]:
    return {"state": classify_address(load_ru_networks(path), destination)}


def resolve_and_classify(networks: list[IPNetwork], destination: str, port: int) -> tuple[str, tuple[int, tuple]]:
    try:
        address = ipaddress.ip_address(destination)
    except ValueError as exc:
        raise DatasetUnavailable("destination must be an Xray-resolved IP address") from exc
    family = socket.AF_INET6 if address.version == 6 else socket.AF_INET
    sockaddr = (str(address), port, 0, 0) if family == socket.AF_INET6 else (str(address), port)
    return classify_address(networks, str(address)), (family, sockaddr)
