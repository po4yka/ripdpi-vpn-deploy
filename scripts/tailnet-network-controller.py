...
def module():
    spec = importlib.util.spec_from_file_location("tailnet_network_promotion", PROMOTION_PATH)
...
def private(path: Path, *, executable=False):
    before = path.lstat()
    if not stat.S_ISREG(before.st_mode) or before.st_uid != os.geteuid() or before.st_nlink != 1 or before.st_mode & (0o022 if executable else 0o077):
        raise ControllerError("private-input-refused")
    fd = os.open(path, os.O_RDONLY | os.O_NONBLOCK | getattr(os, "O_NOFOLLOW", 0))
    after = os.fstat(fd)
    if (before.st_dev, before.st_ino) != (after.st_dev, after.st_ino):
        os.close(fd); raise ControllerError("private-input-refused")
    return fd

def load_config(path: Path):
    fd = private(path)
    try: raw = os.read(fd, MAX + 1)
    finally: os.close(fd)
    try: value = json.loads(raw)
    except (ValueError, UnicodeError): raise ControllerError("config-refused") from None
    required = {"inventory_path", "inventory_name", "contexts", "mode", "promotion_config_path", "provider_target_path", "provider_state_path", "terraform_path", "terraform_sha256", "known_hosts_path", "executor_dir", "candidate_fragment_path"}
    if set(value) != required or value["mode"] not in {"dry-run", "apply"} or not all(isinstance(value[k], str) for k in required - {"contexts", "mode"}):
        raise ControllerError("config-refused")
    return value

def guard(socket_path: Path):
    def invoke(action, value):
        payload = json.dumps({"action": action, "value": value}, sort_keys=True, separators=(",", ":")).encode()
        if len(payload) > MAX: raise p.PromotionError("rollback-uncertain")
        try:
            with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as client:
                client.settimeout(5); client.connect(str(socket_path)); client.sendall(payload); client.shutdown(socket.SHUT_WR); raw = client.recv(MAX + 1)
            result = json.loads(raw)
            if not isinstance(result, dict) or set(result) != {"ok"}: raise ValueError
            return result["ok"]
...
def run(config):
    ...
    aliases = p.fleet_inspection.select_hosts(Path(config["inventory_path"]), [config["inventory_name"]])
    if len(aliases) != 1: raise ControllerError("selection-refused")
    target, state, terraform = (Path(config[k]) for k in ("provider_target_path", "provider_state_path", "terraform_path"))
    ...
    try: candidate = os.read(candidate_fd, MAX + 1)
    ...
    if config["mode"] == "apply":
        ...
        if not token: raise ControllerError("provider-credentials-unavailable")
        adapter.environment_map = {**adapter.environment_map, "UPCLOUD_TOKEN": token}
    def return_path(action, identity):
        if action not in {"forward", "readback"}: raise p.PromotionError("provider-return-path-invalid")
        if action == "forward": return True
        try:
            state_value = json.loads(adapter._command(["state", "pull"]))
            resource = next(item for item in state_value["resources"] if item.get("type") == "upcloud_server" and item.get("name") == "vpn")
            firewall = resource["instances"][0]["attributes"]["firewall"]
        except (KeyError, IndexError, StopIteration, TypeError, ValueError, p.PromotionError):
            raise p.PromotionError("provider-readback-invalid") from None
        if type(firewall) is not bool: raise p.PromotionError("provider-readback-invalid")
        return {**identity, "firewall": firewall}
    adapter.return_path_guard = return_path
    request = {"inventory_path": config["inventory_path"], "inventory_name": config["inventory_name"], "contexts": config["contexts"], "mode": config["mode"], "promotion_config_path": config["promotion_config_path"], "target_identity": {k: adapter.target.value[k] for k in ("inventory_alias", "public_service_address_sha256", "deployable_digest")}, "provider_target_sha256": adapter.target.digest}
    return p.execute(request, adapter, guest=strict_guest(aliases[0], Path(config["known_hosts_path"]), candidate), known_hosts=Path(config["known_hosts_path"]))

def main():
    parser = argparse.ArgumentParser(); parser.add_argument("--config", required=True); args = parser.parse_args()
    try: print(json.dumps(run(load_config(Path(args.config))), sort_keys=True))
    except (ControllerError, p.PromotionError): return 1
    return 0
