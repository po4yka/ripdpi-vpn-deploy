...
class TaskctlContractTest(TaskctlFixture):
    ...
    def test_new_removes_task_when_openspec_scaffold_fails(self) -> None:
        ...
        with self.assertRaisesRegex(taskctl.ContractError, "scaffold new change refused"):
            taskctl.command_new(args)
...
""",
    ...
    def test_later_malformed_merged_incarnation_supersedes_valid_purge(self) -> None:
        ...
        integration_branch = self.git("branch", "--show-current")
        self.git("branch", "malformed-reincarnation")
        self.prepare_simple_terminal(source)
        self.write_board()
        ...
        self.git("switch", "malformed-reincarnation")
        source = self.add_simple_task(task_id=source_task, status="doing")
        self.write_board()
