...
_GROUP_VARS = yaml.safe_load(
    (REPO_ROOT / "ansible" / "group_vars" / "all.yml").read_text()
)
RUNTIME_PORT_DEFAULTS = {
    name: _GROUP_VARS[name]
    for name in (
        "xray_port",
        "xray_fallback_port",
        ...
        contract.check(
            {
                ...
                "actual": [
                    _actual("xray", "tcp", 443),
                    _actual("amneziawg", "udp", 51820),
                ],
            }
        )
        == []
    ...
    findings = contract.check(
        {
            ...
            "actual": [
                _actual("xray", "tcp", 443),
                _actual("xray-fallback", "tcp", 2053),
            ],
        }
    )
        ...
        contract.check(
            {
                ...
                "actual": [
                    _actual("xray", "tcp", 443),
                    _actual("honeypot", "tcp", 4443, enabled=False),
                ],
            }
        )
        == []
...
def test_listener_ports_have_one_runtime_default_and_keep_the_provider_edge_boundary() -> (
    None
):
    """Pre-task rendering consumes group-vars ports; Terraform remains the edge SOT."""
    ports = (REPO_ROOT / "ansible" / "group_vars" / "all.yml").read_text()
    manifest = MANIFEST_TEMPLATE.read_text()
    ...
    for name in (
        ...
        assert ports.count(f"{name}:") == 1
        assert f"{name} | default(" not in manifest

    assert "Terraform edge contract is intentionally independent" in manifest


def test_declared_runtime_port_defaults_render_with_no_consumer_numeric_fallbacks() -> (
    None
):
    """Changing a canonical port must flow through the manifest and consumers."""
    variables = renderer.merge_render_vars()
    overrides = {
        "xray_port": 1443,
        "xray_fallback_port": 12053,
        "nginx_xhttp_public_port": 18443,
        "nginx_xhttp_port": 11085,
        "nginx_xhttp_fallback_port": 12083,
        "hysteria_port": 1443,
        "amneziawg_listen_port": 151820,
    }
    variables.update(overrides)
    variables["amneziawg"]["listen_port"] = overrides["amneziawg_listen_port"]
    rendered = json.loads(renderer.render_template(MANIFEST_TEMPLATE, variables))
    ports = {
        item["role"]: item.get("port")
        for item in rendered
        if item.get("port") is not None
    }
    assert ports["xray"] == overrides["xray_port"]
    assert ports["xray-fallback"] == overrides["xray_fallback_port"]
    assert ports["nginx-xhttp"] == overrides["nginx_xhttp_public_port"]
    assert ports["hysteria"] == overrides["hysteria_port"]
    assert ports["amneziawg"] == overrides["amneziawg_listen_port"]
    ...
    sources = (
        ...
        REPO_ROOT / "ansible/roles/split-hop-ingress/defaults/main.yml",
    )
    numeric_default = __import__("re").compile(
...
def test_installed_ansible_uses_canonical_optional_listener_ports(
    tmp_path: Path,
) -> None:
    """Role templates/defaults follow the same top-level ports as site pre-tasks."""
    executable = shutil.which("ansible-playbook")
    assert executable, "installed Ansible is required for listener port parity"
    rendered_subscription = tmp_path / "subscription.conf"
    rendered_ingress = tmp_path / "split-hop-ingress.conf"
    subscription_defaults = yaml.safe_load(
        (REPO_ROOT / "ansible/roles/subscription-host/defaults/main.yml").read_text()
    )
    ingress_defaults = yaml.safe_load(
        (REPO_ROOT / "ansible/roles/split-hop-ingress/defaults/main.yml").read_text()
    )
    playbook = tmp_path / "ports.yml"
    playbook.write_text(
        yaml.safe_dump(
            [
                {
                    ...
                    "vars": {
                        ...
                        "subscription_port": 18444,
                        "split_hop_ingress_listen_port": 551821,
                        **subscription_defaults,
                        "nginx_xhttp": {"server_name": "sub.example.test"},
                        "vpn": {},
                        **ingress_defaults,
                        "split_hop_ingress_secrets": {
                            "node_a_private_key": "fixture-private",
                            "node_b_public_key": "fixture-public",
                        },
                    },
                    "tasks": [
                        {
                            "ansible.builtin.template": {
                                ...
                                "dest": str(rendered_subscription),
                            }
                        },
                        {
                            "ansible.builtin.template": {
                                "src": str(
                                    REPO_ROOT
                                    / "ansible/roles/split-hop-ingress/templates/split-hop-ingress.conf.j2"
                                ),
                                "dest": str(rendered_ingress),
                            }
                        },
                    ],
                }
    ...
    assert "listen 18444 ssl http2;" in rendered_subscription.read_text()
    assert "ListenPort = 551821" in rendered_ingress.read_text()
...
def test_listener_contract_pre_tasks_run_during_tagged_deploys() -> None:
    play = yaml.safe_load(
        (REPO_ROOT / "ansible" / "playbooks" / "site.yml").read_text()
    )[0]
    required = {
    ...
    tasks = {
        task["name"]: task for task in play["pre_tasks"] if task["name"] in required
    }
...
def test_tagged_convergence_executes_source_preflight_guards(
    tmp_path, case, expected_failure
):
    ...
    shutil.copyfile(
        REPO_ROOT / "ansible/role-tiers.yml", playbooks.parent / "role-tiers.yml"
    )
...
def test_verify_hysteria_uses_configured_udp_port(tmp_path, observed_port, success):
    result, inspected = _run_listener_task(
        tmp_path,
        "ss -lnu",
        {"vpn": {"enable_hysteria": True}, "hysteria_port": 7443},
        "udp",
        observed_port,
    )
...
@pytest.mark.parametrize(
    "fallback, configured_port, observed_port, success",
    [
        ("xray", 2443, 2443, True),
        ("xray", 2443, 443, False),
...
def test_verify_requires_enabled_fallback_listener(
    tmp_path, fallback, configured_port, observed_port, success
):
    variables = {
    ...
    result, inspected = _run_listener_task(
        tmp_path, fallback + "_fallback_port", variables, "tcp", observed_port
    )
    assert inspected, result.stdout + result.stderr
    assert (result.returncode == 0) == success, result.stdout + result.stderr
...
@pytest.mark.parametrize(
    "match", ["ss -lnu", "xray_fallback_port", "nginx_xhttp_fallback_port"]
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "xray_fallback_port",
            {"vpn": {"enable_xray_reality": False}, "xray_fallback_port": 2443},
        ),
        (
            "xray_fallback_port",
            {
                "vpn": {"enable_xray_reality": False},
        ...
        (
            "xray_fallback_port",
            {"vpn": {"enable_xray_reality": False}, "xray_fallback_port": 0},
        ),
        (
            "xray_fallback_port",
            {
        ...
        (
            "nginx_xhttp_fallback_port",
            {"vpn": {"enable_nginx_xhttp": False}, "nginx_xhttp": {}},
        ),
    ],
)
...
@pytest.mark.parametrize(
    "match, variables, protocol, port",
    [
        ("ss -lnu", {"vpn": {"enable_hysteria": True}}, "udp", 443),
        (
            "nginx_xhttp_fallback_port",
            {
                "vpn": {"enable_nginx_xhttp": True},
                "nginx_xhttp": {"fallback_enabled": True},
            },
            "tcp",
            2083,
...
def test_verify_listener_defaults_match_rendered_configuration(
    tmp_path, match, variables, protocol, port
):
...
HOSTCLASS_TASKS = (
    "Xray TCP listener active",
    "nginx-xhttp public TCP listener active",
    "nginx -t",
    "P1 public hostname resolves to its service address",
    "P1 public IPv6 service address is available",
    "P1 public hostname exposes its IPv6 service address",
    "Hysteria service active",
    "Snell configuration is valid",
    "Snell service active",
    "Snell TCP listeners active",
    "AmneziaWG interface up",
)


def _run_hostclass_task_slice(
    tmp_path, names, *, subscription_only, enabled=True, bad_sysctl=False
):
    source = yaml.safe_load((REPO_ROOT / "ansible/playbooks/verify.yml").read_text())[0]
...
"""
    ...
    for task in tasks:
        ...
        if command.get("cmd", "").startswith("/usr/local/bin/sing-box-snell "):
            command["cmd"] = command["cmd"].replace(
                "/usr/local/bin/sing-box-snell", str(tmp_path / "sing-box-snell"), 1
            )
    ...
    variables = {
        ...
        "nginx_xhttp": {"server_name": "fixture.invalid"},
        "vpn_service_address": "192.0.2.7",
...
def test_verify_hostclass_enabled_transports_still_inspect_state(tmp_path):
    result, calls = _run_hostclass_task_slice(
        tmp_path, HOSTCLASS_TASKS, subscription_only=False
    )
    output = result.stdout + result.stderr
    assert result.returncode == 0, output
...
def test_verify_hostclass_disabled_transports_do_not_inspect_state(tmp_path):
    result, calls = _run_hostclass_task_slice(
        tmp_path, HOSTCLASS_TASKS, subscription_only=False, enabled=False
    )
    assert result.returncode == 0, result.stdout + result.stderr
...
def test_verify_hostclass_preserves_shared_hardening_checks(tmp_path, bad_sysctl):
    names = (
        "Cloud-init bootstrap marker exists",
        "nftables config syntax valid",
        "SSH password auth disabled",
        "SSH effective hardening is applied",
        "Sysctl hardening values are applied",
    )
    result, calls = _run_hostclass_task_slice(
        tmp_path, names, subscription_only=True, bad_sysctl=bad_sysctl
    )
    output = result.stdout + result.stderr
