...
def test_word_block_inside_a_hostname_is_not_matched(daemon):
    ...
    line = "... accepted tcp:1.2.3.4:443 [vless-reality -> direct] email: blockchain-user"
    assert daemon["classify_line"](line) is None
...
@pytest.mark.parametrize("ip", ["10.0.0.5", "127.0.0.1", "192.168.1.1",
                                 "172.16.0.1", "172.31.255.254", "0.0.0.0"])
def test_private_ips_are_exempt(daemon, ip):
    assert daemon["is_exempt_ip"](ip) is True


@pytest.mark.parametrize("ip", ["203.0.113.55", "198.51.100.10",
...
def test_source_ip_is_extracted_not_private_destination(daemon):
    ...
    line = ("from tcp:203.0.113.55:52050 accepted tcp:192.168.1.1:80 "
...
def test_below_threshold_does_not_ban(daemon):
    limiter = daemon["RateLimiter"](5, 60, 300)
    line = ("from tcp:198.51.100.20:40001 accepted udp:1.2.3.4:443 "
...
def test_window_expiry_prevents_ban(daemon):
    ...
    limiter = daemon["RateLimiter"](5, 60, 300)
    line = ("from tcp:203.0.113.77:50000 accepted udp:1.2.3.4:443 "
