...
def adapter(monkeypatch):
    ...
    spec = importlib.util.spec_from_file_location(
        "sshd_migrate", FILES / "sshd_migrate.py"
    )
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    monkeypatch.setattr(
        module.Runtime, "boot_id", lambda self: "00000000-0000-4000-8000-000000000001"
    )
    return module


def test_boot_recovery_has_no_reload_and_normal_adapter_uses_fixed_command(
    adapter, monkeypatch
):
    calls = []
...
def test_recovery_requires_enabled_timer_and_real_ssh_dependencies(
    adapter, monkeypatch
):
    values = installed_properties(adapter)
    monkeypatch.setattr(adapter, "_command", systemd_fixture(values))
    monkeypatch.setattr(
        adapter, "_validate_installation", lambda: Path("/generation") / ("a" * 64)
    )
    ...
    values[("show", "ssh.socket", "--property=Requires", "--value")] = (
        "vpn-sshd-boot-recover.service"
    )
    values[("show", "vpn-sshd-recover.timer", "--property=Requires", "--value")] = (
        "other.service"
    )
    assert not adapter.Runtime().recovery_ready()
...
def test_request_is_fixed_shape_and_requires_two_connection_contexts(adapter):
    with pytest.raises(adapter.TransactionError):
        adapter.validate_request(
            "prepare",
            {
                "intent": "sshd-ownership",
                "contexts": [],
                "timeout": 180,
                "check_mode": False,
                "bundle_generation": "a" * 64,
                "config_root": "/tmp",
            },
        )
    with pytest.raises(adapter.TransactionError):
        adapter.validate_request(
            "prepare", {"intent": "sshd-ownership", "contexts": [], "timeout": 180}
        )
    with pytest.raises(adapter.TransactionError):
        adapter.validate_request(
            "apply", {"generation": "x", "nonce": "y", "command": "true"}
        )
...
def test_prepare_never_infers_intent_or_accepts_an_alternate_candidate_surface(
    adapter, extra
):
...
def test_prepare_requires_typed_check_mode_and_exact_bundle_generation(
    adapter, field, value
):
    request = {
        "intent": "sshd-ownership",
...
def installed_properties(adapter):
    ...
    for unit in adapter.UNIT_HASHES:
        ...
        values[("show", unit, "--property=FragmentPath", "--value")] = (
        ...
        if unit.endswith(".service"):
            ...
            values[
                ("show", unit, "--property=ExecMainStartTimestampMonotonic", "--value")
            ] = "100"
            values[
                ("show", unit, "--property=ExecMainExitTimestampMonotonic", "--value")
            ] = "200"
    ...
    for unit in ("ssh.service", "ssh.socket", "vpn-sshd-recover.timer"):
        for prop in ("Requires", "After"):
            values[("show", unit, "--property=" + prop, "--value")] = (
                "vpn-sshd-boot-recover.service other.service"
            )
    return values


@pytest.mark.parametrize(
    "unit", ["vpn-sshd-boot-recover.service", "vpn-sshd-recover.service"]
)
@pytest.mark.parametrize(
    "property_name,value",
    [("ActiveState", "failed"), ("Result", "resources"), ("LoadState", "error")],
)
def test_failed_recovery_worker_is_not_ready_even_with_active_timer(
    adapter, monkeypatch, unit, property_name, value
):
    values = installed_properties(adapter)
    monkeypatch.setattr(adapter, "_command", systemd_fixture(values))
    monkeypatch.setattr(
...
def test_never_executed_worker_is_not_readiness_evidence(adapter, monkeypatch, unit):
    ...
    assert adapter.Runtime().recovery_ready()
    values[("show", unit, "--property=ExecMainStartTimestampMonotonic", "--value")] = (
        "0"
    )
    ...
    values[("show", unit, "--property=ExecMainExitTimestampMonotonic", "--value")] = (
        "200"
    )
...
@pytest.mark.parametrize(
    "action,expected", [("recover", 75), ("boot-recover", 1), ("status", 1)]
)
def test_only_periodic_lock_contention_is_deferred_not_failed(
    adapter, monkeypatch, capsys, action, expected
):
    import json
...
def test_unapproved_loaded_recovery_units_cannot_arm(adapter, monkeypatch, fault):
    ...
    if fault == "dropin":
        values[
            ("show", "vpn-sshd-recover.service", "--property=DropInPaths", "--value")
        ] = "/run/systemd/system/vpn-sshd-recover.service.d/override.conf"
    elif fault == "transient":
        values[
            ("show", "vpn-sshd-recover.timer", "--property=FragmentPath", "--value")
        ] = "/run/systemd/transient/vpn-sshd-recover.timer"
    else:
        values[
            (
                "show",
...
def test_boot_unit_provides_privsep_directory_before_ssh_service(adapter):
    boot = (
        ROOT / "ansible/roles/baseline/templates/vpn-sshd-boot-recover.service"
    ).read_text()
...
def test_installer_has_exact_host_guard_without_importing_baseline_or_site(adapter):
    ...
    play = yaml.safe_load(
        (ROOT / "ansible/playbooks/install-sshd-recovery.yml").read_text()
    )[0]
    ...
    assert all(
        not any(
            key.endswith(("include_role", "import_role", "import_playbook"))
            for key in task
        )
...
def test_real_planner_transaction_and_installed_policy_round_trip(
    adapter, request, monkeypatch, configuration, intent, terminal
):
    import os
    ...
    with tempfile.TemporaryDirectory(prefix=".sshd-state-", dir=config.parent) as state:
        ...
        before = {
            ...
            for path in [config / "sshd_config", *(config / "sshd_config.d").iterdir()]
        }
        candidate = (
            {"hardening": BASELINE_HARDENING} if intent == "sshd-baseline" else {}
        )
        receipt = engine.prepare(
            intent=intent, contexts=CONTEXTS, timeout=120, **candidate
        )
        assert receipt["status"] == "prepared"
        engine.apply(receipt["generation"], receipt["nonce"])
        assert (
            b"PasswordAuthentication"
            not in (config / "sshd_config.d/20-ansible-hardening.conf").read_bytes()
        )
        ...
        if terminal == "committed":
            assert (
                engine.confirm(
                    receipt["generation"], receipt["nonce"], receipt["snapshot_digest"]
                )["status"]
                == terminal
            )
        runtime.now += 121
        assert engine.recover()["status"] == terminal
        assert {path: path.read_bytes() for path in before} == (
            before if terminal == "rolled_back" else after
        )
...
def test_units_dispatch_immutable_bundle_and_write_only_sshd_fragments(adapter):
...
def test_installation_uses_one_publisher_not_sequential_live_modules(adapter):
    ...
    assert any(
        "/staging/" in task.get("ansible.builtin.copy", {}).get("dest", "")
        for task in tasks
    )
    assert all(
        task.get("ansible.builtin.copy", {}).get("dest")
        != "/usr/local/lib/vpn-sshd/{{ item }}"
        for task in tasks
    )
    bootstrap = next(
        ...
        == "/usr/local/lib/vpn-sshd/sshd_bundle.py"
    )
...
def test_installed_generation_and_controlled_links_are_checked(
...
def systemd_fixture(values, calls=None):
    ...
    def command(args, **kwargs):
        ...
        if args[1] == "show" and args[-1] != "--value":
            ...
            return "\n".join(
                name + "=" + values[("show", unit, "--property=" + name, "--value")]
                for name in fields
            )
...
def activation_fixture(adapter, monkeypatch):
    ...
    monkeypatch.setattr(
        runtime, "boot_id", lambda: "00000000-0000-4000-8000-000000000001"
    )
    return runtime, values, now, calls


def test_activation_replaces_old_busy_result_with_one_fresh_completed_execution(
    adapter, monkeypatch
):
...
def worker_execution(
    values, *, status="75", started=1500000, exited=1600000, inflight=False
):
    worker = "vpn-sshd-recover.service"
    fields = {
        "ExecMainPID": "202",
        "ExecMainStartTimestampMonotonic": str(started),
        "ExecMainExitTimestampMonotonic": str(exited),
        "ExecMainCode": "1",
...
def test_fresh_proof_survives_only_its_later_periodic_self_contention(
    adapter, monkeypatch, inflight
):
    runtime, values, now, calls = activation_fixture(adapter, monkeypatch)
    proof = runtime.activation_recovery()
    acquired = 1400000
    ...
    if inflight:
        values[("show", "vpn-sshd-recover.timer", "--property=SubState", "--value")] = (
            "running"
        )
    now["us"] = 1700000
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "expired-proof",
        "old-execution",
        "equal-lock-timestamp",
        "future-execution",
        "real-failure",
        "inflight-before-lock",
        "missing-pid",
        "generation",
        "boot",
        "boot-failed",
...
def test_activation_fence_refuses_ambiguous_or_changed_capability(
    adapter, monkeypatch, fault
):
    runtime, values, now, calls = activation_fixture(adapter, monkeypatch)
    proof = runtime.activation_recovery()
    acquired = 1400000
    ...
    if fault == "no-proof":
        ...
        monkeypatch.setattr(
            adapter, "_validate_installation", lambda: Path("/generation") / ("b" * 64)
        )
    elif fault == "boot":
        monkeypatch.setattr(
            runtime, "boot_id", lambda: "00000000-0000-4000-8000-000000000002"
        )
    elif fault == "boot-failed":
        change("vpn-sshd-boot-recover.service", "Result", "resources")
    elif fault == "boot-reexecuted":
        change(
            "vpn-sshd-boot-recover.service", "ExecMainStartTimestampMonotonic", "150"
        )
...
def test_expired_command_deadline_never_starts_a_root_process(adapter, monkeypatch):
    ...
    with pytest.raises(adapter.TransactionError):
        adapter._command(
            ["/usr/bin/systemctl", "start", "vpn-sshd-recover.service"], deadline=0
        )
...
@pytest.mark.parametrize(
    "fault", ["override", "boot-never-run", "boot-busy", "boot-failed", "disabled"]
)
def test_activation_preflight_refuses_before_any_service_start(
    adapter, monkeypatch, fault
):
    runtime, values, now, calls = activation_fixture(adapter, monkeypatch)
    unit = "vpn-sshd-boot-recover.service"
    name, value = {
        "override": ("DropInPaths", "/run/untrusted.conf"),
        "boot-never-run": ("ExecMainStartTimestampMonotonic", "0"),
        "boot-busy": ("ExecMainStatus", "75"),
        "boot-failed": ("Result", "resources"),
...
@pytest.mark.parametrize(
    "fault",
    ["cached", "busy", "failed", "future", "inflight", "command-failed", "expired"],
)
def test_one_start_must_produce_fresh_completed_zero_without_retry(
    adapter, monkeypatch, fault
):
    runtime, values, now, calls = activation_fixture(adapter, monkeypatch)
    original = adapter._command
    ...
    def start(args, **kwargs):
        ...
        if args[1:] == ["start", "vpn-sshd-recover.service"]:
            ...
            if fault == "expired":
                ...
                worker_execution(
                    values,
                    status="75" if fault == "busy" else "0",
                ...
                if fault == "future":
                    worker_execution(
                        values, status="0", started=1400000, exited=1500000
                    )
        return result
...
@pytest.mark.parametrize("inflight", [False, True])
def test_real_planner_apply_survives_periodic_race_after_second_flock(
    adapter, config, monkeypatch, inflight
):
    import os
    import tempfile
    ...
    with tempfile.TemporaryDirectory(
        prefix=".sshd-race-state-", dir=config.parent
    ) as state:
        engine = adapter.transaction.Transaction(config, Path(state), runtime)
        receipt = engine.prepare(
            intent="sshd-ownership", contexts=CONTEXTS, timeout=120
        )
        ...
        def raced_fence(proof, acquired):
            ...
            with pytest.raises(adapter.TransactionError, match="busy"):
                engine.recover()
            worker_execution(
                values, started=acquired + 100, exited=acquired + 200, inflight=inflight
            )
            if inflight:
                values[
                    ("show", "vpn-sshd-recover.timer", "--property=SubState", "--value")
                ] = "running"
            now["us"] = acquired + 300
        ...
        assert (
            engine.apply(receipt["generation"], receipt["nonce"])["status"] == "applied"
        )
        assert (
            b"PasswordAuthentication"
            not in (config / "sshd_config.d/20-ansible-hardening.conf").read_bytes()
...
def test_recovery_command_enforces_deadline_with_a_real_child(adapter):
    ...
    with pytest.raises(adapter.TransactionError, match="command-timeout"):
        adapter._command(
            [sys.executable, "-c", "import time; time.sleep(5)"], deadline=started + 0.1
        )
...
def test_prior_periodic_failure_cannot_be_hidden_by_successful_restart(
    adapter, monkeypatch, fault
):
    runtime, values, now, calls = activation_fixture(adapter, monkeypatch)
    worker = "vpn-sshd-recover.service"
...
def test_future_execution_metrics_never_prove_readiness_or_allow_restart(
    adapter, monkeypatch, unit, future
):
    runtime, values, now, calls = activation_fixture(adapter, monkeypatch)
    values[
        ("show", "vpn-sshd-recover.service", "--property=ExecMainStatus", "--value")
    ] = "0"
    values[("show", unit, "--property=ExecMainExitTimestampMonotonic", "--value")] = (
        "3000000"
    )
