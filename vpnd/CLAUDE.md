...
Pass `--token <tok>` (from `make issue-sub-token`) to use an opaque subscription URL; the flag is required and the token is validated against `^[A-Za-z0-9_-]+$` before use.
