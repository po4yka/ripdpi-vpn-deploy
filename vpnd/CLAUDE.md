...
Pass `--token <tok>` (from `make issue-sub-token`) to use an opaque subscription URL instead of the enumerable client name; token is validated against `^[A-Za-z0-9_-]+$` before use.
