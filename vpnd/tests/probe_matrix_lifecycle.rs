...
fn wait_for_checkpoint(path: &Path) -> Value {
    ...
    loop {
        ...
        assert!(
            Instant::now() < deadline,
            "no completed-tick checkpoint before test deadline"
        );
        std::thread::sleep(Duration::from_millis(20));
    }
}
...
fn journal_records(path: &Path) -> Vec<Value> {
    std::fs::read_to_string(companion(path, ".jsonl"))
        .unwrap()
        .lines()
...
#[test]
fn normal_run_checkpoints_private_report_and_jsonl_journal() {
    ...
    let journal = companion(&output, ".jsonl");
    let lock = companion(&output, ".lock");
    let records = journal_records(&output);
    assert!(records.len() >= 3, "initial, tick, and terminal records");
    assert!(records.iter().all(|record| record["schema_version"] == 3));
    assert_eq!(records.first().unwrap()["completed"], false);
    assert_eq!(records.first().unwrap()["interrupted"], false);
    assert_eq!(records.last().unwrap()["completed"], true);
    assert_eq!(records.last().unwrap()["interrupted"], false);
    assert_eq!(
        records
            .iter()
            .filter(|record| !record["control"].is_null())
            .count(),
        report["controls"].as_array().unwrap().len()
    );
    let journal_cells: Vec<Value> = records
        .iter()
        .flat_map(|record| record["cells"].as_array().unwrap().clone())
        .collect();
    assert_eq!(&journal_cells, report["cells"].as_array().unwrap());
    for path in [&output, &journal, &lock] {
        assert_private(path);
    }
}

#[test]
fn signal_during_scheduled_wait_flushes_prior_ticks_without_synthetic_cells() {
    let fixture = Fixture::new(OK, OK);
    let output = fixture.root.path().join("report.json");
    ...
    let checkpoint = wait_for_checkpoint(&output);
    let expected_controls = checkpoint["controls"].as_array().unwrap().len();
    let expected_cells = checkpoint["cells"].as_array().unwrap().len();
    assert!(Command::new("kill")
        .args(["-INT", &child.0.id().to_string()])
        .status()
        .unwrap()
        .success());
    assert_eq!(wait(&mut child.0).code(), Some(130));
    ...
    assert_eq!(report["cells"].as_array().unwrap().len(), expected_cells);
    let terminal = journal_records(&output).pop().unwrap();
    assert_eq!(terminal["completed"], false);
    assert_eq!(terminal["interrupted"], true);
    assert!(terminal["control"].is_null());
    assert!(terminal["cells"].as_array().unwrap().is_empty());
}

#[test]
fn signals_during_cells_flush_partial_evidence_and_reclaim_descendants() {
    for (signal, exit_code) in [("INT", 130), ("TERM", 143)] {
        let fixture = Fixture::new(OK, "sh cell.sh");
        std::fs::write(
            fixture.root.path().join("cell.sh"),
            r#"if [ "$TARGET_ID" = pair-dual ]; then
    printf '%s\n' '{"verdict":"blocked"}'
...
"#,
        ...
        let deadline = Instant::now() + Duration::from_secs(5);
        while !fixture.root.path().join("quick-cell-complete").exists()
            || !fixture.root.path().join("slow-cell.pids").exists()
        {
            assert!(
                child.0.try_wait().unwrap().is_none(),
                "CLI exited before both cell boundaries were observed"
            );
            assert!(Instant::now() < deadline, "cells never became ready");
            std::thread::sleep(Duration::from_millis(20));
        }
        let pids = std::fs::read_to_string(fixture.root.path().join("slow-cell.pids")).unwrap();
        assert!(pids.lines().all(running));
        ...
        assert!(Command::new("kill")
            ...
            .unwrap()
            .success());
        assert_eq!(wait(&mut child.0).code(), Some(exit_code));
        ...
        assert_eq!(cells[0]["target_id"], "pair-dual");
        assert_eq!(cells[0]["verdict"], "blocked");
        assert_eq!(cells[1]["target_id"], "pair-split");
        assert_eq!(cells[1]["verdict"], "unknown");
        assert_eq!(cells[1]["error_kind"], "interrupted");

        let records = journal_records(&output);
        assert_eq!(records.last().unwrap()["completed"], false);
        assert_eq!(records.last().unwrap()["interrupted"], true);
        let journal_cells: Vec<Value> = records
            .iter()
            .flat_map(|record| record["cells"].as_array().unwrap().clone())
            .collect();
        assert_eq!(&journal_cells, cells);
        let reaped = Instant::now() + Duration::from_secs(3);
        while pids.lines().any(running) && Instant::now() < reaped {
            std::thread::sleep(Duration::from_millis(20));
        }
        assert!(
...
#[test]
fn same_output_sessions_are_exclusive_and_lock_survives_reuse() {
    ...
    let checkpoint = std::fs::read(&output).unwrap();
    let second = fixture
        .command("1s")
        .stderr(Stdio::piped())
        .output()
    ...
    assert_eq!(std::fs::read(&output).unwrap(), checkpoint);
    ...
    assert!(fixture.command("1s").status().unwrap().success());
    assert_eq!(std::fs::metadata(&lock).unwrap().ino(), metadata.ino());
}

#[test]
fn output_names_with_shared_stems_keep_distinct_companions() {
    let fixture = Fixture::new(OK, OK);
    let json_output = fixture.root.path().join("report.json");
    let text_output = fixture.root.path().join("report.txt");
    assert!(fixture
        .command_at("1s", &json_output)
        .status()
        .unwrap()
        .success());
    assert!(fixture
        .command_at("1s", &text_output)
        .status()
        .unwrap()
        .success());

    for suffix in [".jsonl", ".lock"] {
        let json_companion = companion(&json_output, suffix);
        let text_companion = companion(&text_output, suffix);
        assert_ne!(json_companion, text_companion);
        assert!(json_companion.exists());
        assert!(text_companion.exists());
    }
}

#[test]
fn unsafe_output_locks_and_reserved_suffixes_fail_before_make() {
    for suffix in ["jsonl", "lock"] {
        let fixture = Fixture::new("touch invoked", "touch invoked");
        let output = fixture.root.path().join(format!("report.{suffix}"));
        assert!(!fixture
    ...
    for case in ["permissive", "nonempty", "symlink"] {
        let fixture = Fixture::new("touch invoked", "touch invoked");
        let output = fixture.root.path().join("report.json");
        let lock = companion(&output, ".lock");
        match case {
            ...
            "symlink" => {
                let target = fixture.root.path().join("lock-target");
...
#[test]
fn failed_interrupted_checkpoint_preserves_last_valid_report_and_exits_one() {
    let fixture = Fixture::new(OK, OK);
    let output = fixture.root.path().join("report.json");
    let mut child = Running(
        fixture
    ...
    wait_for_checkpoint(&output);
    let checkpoint = std::fs::read(&output).unwrap();
    let root_mode = std::fs::metadata(fixture.root.path())
        .unwrap()
        .permissions()
        .mode()
        & 0o777;
